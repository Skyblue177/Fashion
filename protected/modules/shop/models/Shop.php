<?php
	...
	class Shop {
		...
		public static function pricingInfo() {
			...
			echo Shop::t('Tất cả giá đã bao gồm thuế VAT') . '<br />';
			echo Shop::t('Tất cả giá bao gồm chi phí vận chuyển');
			echo '</p>';
...
/*		public function getCustomer() {
		...
		public static function getPriceTotal() {
			...
			$price_total = Shop::t('Tổng giá: {total}', array(
						'{total}' => Shop::priceFormat($price_total),
						)); 
			$price_total .= '<br />';
			$price_total .= Shop::t('Tất cả giá đã bao gồm thuế VAT: {vat}', array(
						'{vat}' => Shop::priceFormat($tax_total))) . '<br />';
			$price_total .= Shop::t('Tất cả giá bao gồm chi phí vận chuyển') . '<br />';

			return $price_total;
		}
		
		public static function getQtyCart() {
			$qty = 0;
			foreach(Shop::getCartContent() as $product)  {
				$qty++;
			}
